...
local function getMyPlot()
	...
	for _, plot in pairs(plotsFolder:GetChildren()) do
		...
		local ownerObj = plot:FindFirstChild("Owner") or plot:FindFirstChild("OwnerValue") or plot:FindFirstChild("Player")
		...
		if plot.Name:lower() == pName or plot.Name:lower() == "plot_" .. pName then
			return plot
		end
	end
	return nil
...
local function isPlayerInTrainingArea()
	...
	local xzDistance = (Vector3.new(hrp.Position.X, 0, hrp.Position.Z) - Vector3.new(pos.X, 0, pos.Z)).Magnitude
	local yDistance = math.abs(hrp.Position.Y - pos.Y)

	return (xzDistance <= 20 and yDistance <= 15)
end
...
local function getSafeZoneCFrame()
	local line = Workspace:FindFirstChild("Line") 
		...
		or Workspace:FindFirstChild("SafeLine")
		or Workspace:FindFirstChild("Border")

	if not line then
		for _, v in pairs(Workspace:GetChildren()) do
			if v:IsA("BasePart") and (v.Name:lower():find("line") or v.Name:lower():find("safe")) then
				line = v
				break
			end
		end
	...
	if myPlot then
		local ph = placeholder or myPlot:FindFirstChildOfClass("BasePart")
		if ph then
			local pos = ph:IsA("Model") and ph:GetPivot() or ph.CFrame
			return pos * CFrame.new(0, 3, -12)
		end
	end
...
local function getSortedEggs()
	local spawnedItems = Workspace:FindFirstChild("SpawnedItems") or Workspace
	local eggList = {}
...
local function stealBestEgg()
	if isStealingRunning then return end
	isStealingRunning = true
	...
	releaseLPM()
	local finalSafe = getSafeZoneCFrame()
	if finalSafe then tweenTo(finalSafe, 280) end
	isStealingRunning = false
end
...
local function getX2SpeedButton()
	...
	if speedEffect then
		...
		if frame then
			local btn = frame:FindFirstChildOfClass("TextButton") or frame:FindFirstChildOfClass("ImageButton") or frame:FindFirstChild("Button", true)
...
local function clickGuiObject(guiObject)
	...
	local targetBtn = guiObject:IsA("GuiButton") and guiObject or guiObject:FindFirstChildWhichIsA("GuiButton", true)
	if not targetBtn then targetBtn = guiObject end
